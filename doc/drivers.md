...
`main()`のUSER CODEから`app_init()`、メインループから`app_process()`を呼ぶ。起動時はUART4受信、ADC取得、両CANの受信を開始し、ブザーはCCR=0の停止状態にする。起動ログをUART4へ1回送信する。電源ON/OFF・保護値・リセットなどのCAN指令は自動送信しない。初期化失敗は`Error_Handler()`へ伝える。
...
- `board_debug_uart_printf()`は容量制限付き整形。整形用とDMA送信用の領域を分け、送信中のデータを上書きしない。浮動小数点printfの有効化は行っていない。
...
起動後、端末から`?`、CR、LFのいずれかを送ると、ADC有効性・生値・SW判定値・ADCエラー数・両CANの受信数／RX破棄数／bus-off状態変化数を返す。SW判定値は0=INVALID、1=NONE、2=中央、3=後、4=右、5=前、6=左。表示は読取り専用で、旧ゲイン操作やアクチュエーター操作コマンドは実装していない。複数の表示要求はまとめて1回の応答にする。
...
DMAの割当・ハンドル連携・NVIC・IRQ入口・HALコールバックは`Application/Board`の実行時初期化が所有する。`.ioc`にはDMAチャネル／追加NVICの有効化を登録していないため、`.ioc`だけでは起動後の状態を表さない。ADCの連続DMA要求は`.ioc`のADC設定に含まれるが、実際のDMA転送開始は`board_user_adc_init()`が行う。
