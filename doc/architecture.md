...
機体制御の入口は、2026-09-14のユーザー指定によりTIM6割り込みから500 Hz（2 ms）で直接呼び出す。優先度は2で、CANの5/6、ADCの8、UART4の9、TX DMAの10より高い。制御演算とアクチュエーター出力は未実装で、現在は`app_control_step()`で取得済みSWを取り込む。mainへ実行要求フラグを渡して制御する方式にはしない。

UARTの整形関数名は`p()`とし、main側で常時10 Hz実行する。`p()`はIRQからの呼出しを整形前に拒否する。ログは送信待ちで制御を停止せず、遅延時の追い掛け出力を行わない。

割り込み側は転送完了、受信データ格納、時刻やイベントの記録を基本とする。HALコールバックは周辺機能ごとに`Board`の一箇所が所有し、同じコールバックを各ドライバへ重複定義しない。上位への通知はポーリング可能な状態・キューを基本とする。

割り込みとメイン処理で共有するデータは所有者と更新タイミングを明記する。通常ドライバの排他はBASEPRI=優先度5とし、優先度2の制御IRQを止めない。そのため、制御IRQからCAN・UART・電源状態のmain専用APIを呼ばない。制御IRQから読むADCは二重バッファの完成側を公開し、低優先度ADCの書込み途中を見せない。今後CAN帰還や出力を制御IRQへ接続する場合も、同様の公開方式または専用の受渡しを実装する。DMAでは送信完了までバッファを保持し、固定長キューには満杯時の扱いと検出手段を設ける。
